  ...
  return (
    <div className="min-h-screen bg-[rgb(17,24,39)] text-[rgb(243,244,246)] relative overflow-x-clip">
      ...
      <div className="lg:pl-80 flex flex-col min-h-screen">
        {/* Header */}
        <header className="bg-[rgb(31,41,55)] border-b border-[rgb(55,65,81)] h-16 flex items-center px-4 lg:px-8 sticky top-0 z-10">
          {/* Mobile Menu Button */}
          <button
            onClick={(e) => {
              e.preventDefault()
            ...
            className="lg:hidden text-[rgb(217,119,6)] hover:text-[rgb(217,119,6)]/80 transition-colors mr-4"
          >
              ...
              <>
                ...
                <div className="absolute right-0 top-12 w-56 bg-[rgb(31,41,55)] rounded-lg shadow-xl z-50 border border-[rgb(55,65,81)]">
                  ...
                  <div className="border-t border-[rgb(55,65,81)] p-1">
                    ...
                    >
                      {theme === "dark" ? (
                        <Sun size={20} className="text-[rgb(245,158,11)]" />
                      ) : (
                        <Moon size={20} className="text-[rgb(245,158,11)]" />
                      )}
                      <span className="flex-1 text-left">
                        {theme === "dark" ? "Включить светлый режим" : "Включить темный режим"}
                      </span>
                    </button>
                    <button
                    ...
                    {accountItems.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={() => setIsMenuOpen(false)}
                          className={`flex items-center gap-3 px-3 py-2 mx-1 rounded-lg transition-colors ${
                            isActive ? "bg-[rgb(217,119,6)] text-white" : "text-gray-300 hover:bg-gray-700"
                          }`}
                        >
                          <IconComponent size={16} />
                          <span className="text-sm font-medium">{item.label}</span>
        ...
        <main className="flex-1 p-4 lg:p-8">
          <h1 className="text-2xl font-bold tracking-tight mb-6 text-white">{title}</h1>
          {children}
        </main>
      </div>
    </div>
